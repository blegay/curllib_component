...
If ($vl_nbParam>2)
	...
	If ((Length:C16($vt_proxyAddress)>0) & ($vl_proxyPort>0))
		...
		CURL__moduleDebugDateTimeLine (6;Current method name:C684;"PROXY : \""+$vt_proxyUrl+"\"")
		  //AJOUTER À TABLEAU($tl_keys;CURLOPT_PROXY)
		  //AJOUTER À TABLEAU($tt_values;"http://"+$vt_proxyAddress+":"+Chaîne($vl_proxyPort))
		...
		If ((Length:C16($vt_proxyLogin)>0) & (Length:C16($vt_proxyPassword)>0))
			...
			CURL__moduleDebugDateTimeLine (6;Current method name:C684;"PROXYUSERPWD : \""+DBG_passwordDebug ($vt_loginPasswordCombined)+"\"")
			  ...
			  //AJOUTER À TABLEAU($tl_keys;CURLOPT_PROXYUSERPWD)
			  //AJOUTER À TABLEAU($tt_values;CURL__proxyLoginPasswordCombine ($vt_proxyLogin;$vt_proxyPassword))
		End if 
		...
		$vb_tls:=True:C214  //(Sous chaîne($vt_url;1;8)="https://")  //($vt_url="https://@")
		  ...
		  // https://curl.haxx.se/libcurl/c/CURLOPT_HTTPPROXYTUNNEL.html
		...
		If ($vb_tls)
			CURL__optionsObj ($vo_curlOptions;"HTTPPROXYTUNNEL";1)
			CURL__moduleDebugDateTimeLine (6;Current method name:C684;"HTTPPROXYTUNNEL : 1")
			  //AJOUTER À TABLEAU($tl_keys;CURLOPT_HTTPPROXYTUNNEL)
			  //AJOUTER À TABLEAU($tt_values;"1")
		End if 
