...
C_OBJECT:C1216($1;$vo_transferInfo)
  //C_OBJECT($2;$vo_userInfos)
  //C_TEXT($1;$vt_transferInfoJson)
...
If ($vo_transferInfo#Null:C1517)
	  //If (Length($vt_transferInfoJson)>0)
	  //<Modif>
	...
	End if 
	  ...
	  //<Modif> Bruno LEGAY (BLE) (04/01/2022)
	  //C_OBJECT($vo_transferInfo)
	  //$vo_transferInfo:=JSON Parse($vt_transferInfoJson)
	  //<Modif>
					...
					Case of 
						...
						: (($vr_contentLengthDownload>0) & ($vr_speedDownload>0))
							...
							If ($vr_progress>0)
								...
								$vt_message:=$vt_message+", ETA : "+UTL_timeleftString (Round:C94($vr_estimatedTimeleft;0))
							End if 
	...
	If (False:C215)
		  //%T-
		  //<Modif> Bruno LEGAY (BLE) (04/01/2022)
		SET TEXT TO PASTEBOARD:C523(JSON Stringify:C1217($vo_transferInfo;*))
